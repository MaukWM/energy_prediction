...
def make_attention_prediction(E, D, previous_timesteps_x, previous_y, n_output_timesteps):
    ...
    # Expand the previous timesteps, we must make the input a batch (going from shape (100, 149) to (1, 100, 149) for example)
    ...
    attention_weights = np.reshape(np.stack(attention_weights), newshape=(seq_len_out, E.get_layer("last_conv_layer").output_shape[1])).transpose()
    ...
    for i in range(intermediates):
        ...
        if save_weights:
            if save_weights_path:
                ...
                encdecmodel.save_weights("as2s1dc-l{0}-ss{1}-tl{2:.3f}-vl{3:.3f}-i{4}-o{5}-e{6}-seq2seq.h5".format(str(learning_rate),
                                                                                        str(state_size),
